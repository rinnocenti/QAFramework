...
namespace Immersive.Framework.Editor.Editor.Authoring
{
    ...
    internal sealed class ActivityCycleResetTriggerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField("Activity Cycle Reset Trigger", EditorStyles.boldLabel);
            EditorGUILayout.HelpBox(
                "Requests an Activity Cycle Reset through the framework runtime. This is a cycle-level reset only: it does not reset a specific object, component, Player, Actor, pool, save snapshot or scene reload.",
                MessageType.Info);
        ...
        private void DrawRuntimeResult()
        {
            ...
            var trigger = target as ActivityCycleResetTrigger;
            if (trigger == null)
            {
                return;
            }
        ...
        private static MessageType ResolveRuntimeMessageType(ActivityCycleResetTrigger trigger)
        {
        ...
        private static void DrawReasonGuardrail(SerializedProperty reason)
        {
            ...
            {
                EditorGUILayout.HelpBox(
                    "No custom reason set. Runtime diagnostics will use the default Activity Cycle Reset reason.",
                    MessageType.Info);
                return;
            }
            ...
            {
                EditorGUILayout.HelpBox(
                    "The reason contains object/component/player/actor/pool/save/reload vocabulary. This trigger only requests Activity Cycle Reset; use cycle-oriented wording to avoid confusing this with future local reset phases.",
                    MessageType.Warning);
            }
        }
    }
