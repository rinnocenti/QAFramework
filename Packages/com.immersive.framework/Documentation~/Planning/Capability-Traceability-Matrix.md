...
| Capacidade | Decisão | Fase original | Fase revisada | Corte | Status package | Status roadmap | Prioridade | Bloqueadores | Risco | Observação |
|---|---|---|---|---|---|---|---|---|---|---|
| Runtime scope root | Redesenhar como `RuntimeScopeRoot` com registry | F8 | F8 | IF-FW-ROAD-8D | F8D closed / PASS | Coberto | Foundation avançada | Runtime ownership primitives + RuntimeContentHandle | Risco controlado até F8D; release ainda pendente | Root lógico e registry interno existem; ainda sem GameObject hierarchy root real. |
| Runtime root registry | Criar `RuntimeRootRegistry` | F8 | F8 | IF-FW-ROAD-8D | F8D closed / PASS | Coberto | Foundation avançada | RuntimeContentOwner + RuntimeScopeRoot | Risco controlado até F8D; owner runtime ainda pendente | Registry interno lógico existe; não cria fallback root e não é API pública. |
| Runtime content runtime | Criar owner interno do registry | F8 | F8 | IF-FW-ROAD-8E | Pendente / NEXT | Coberto | Foundation avançada | RuntimeRootRegistry lógico F8D | Risco de registry sem dono lifecycle | F8D1 define este como próximo corte obrigatório antes de request/result. |
| Runtime scope context | Passar contexto explícito sem provider global | F8 | F8 | IF-FW-ROAD-8E | Pendente / NEXT | Coberto | Foundation avançada | RuntimeContentRuntime | Risco de virar service locator se mal desenhado | Contexto deve ser criado pelo lifecycle owner e passado à operação; materializer não consulta estado global. |
| Lifecycle root integration | Integrar roots a Route/Activity lifecycle | F8 | F8 | IF-FW-ROAD-8F | Pendente | Coberto | Foundation avançada | RuntimeContentRuntime + RuntimeScopeContext | Risco de orphan se atrasar | Route/Activity devem criar/encerrar roots antes do prefab materializer. |
| Transition guard / scoped cancellation | Formalizar rejeição/cancelamento de runtime content | F8 | F8 | IF-FW-ROAD-8H | Pendente | Coberto | Foundation avançada | Lifecycle root integration | Risco de async fantasma e stale handle | Operação cancelada não registra handle ativo; root releasing/released rejeita materialização. |
| Materialization request/result | Preservar como `RuntimeMaterializationRequest/Result` | F8 | F8 | IF-FW-ROAD-8G | Pendente / Realinhado por F8D1 | Coberto | Foundation avançada | RuntimeContentRuntime + RuntimeScopeContext + lifecycle root integration | Risco se entrar antes do owner/context | F8D1 move request/result para depois de RuntimeContentRuntime e Lifecycle root integration. |
| Runtime content handle | Criar `RuntimeContentHandle` | F8 | F8 | IF-FW-ROAD-8C | F8C closed / PASS | Coberto | Foundation avançada | Runtime ownership primitives | Risco controlado; execution ainda pendente | Handle passivo existe com state transitions e release diagnostics; release real fica para F8J. |
| Prefab materializer | Preservar como `PrefabContentMaterializer` | F8 | F8 | IF-FW-ROAD-8I | Pendente / Realinhado por F8D1 | Coberto | Foundation avançada | RuntimeContentRuntime + Lifecycle root integration + Request/Result + transition guard | Risco se criar prefab antes do owner/root lifecycle | Primeiro materializer deve ser local e simples; sem Addressables, Pool, Content Anchor ou consumers. |
| Runtime release policy | Criar release execution por escopo | F8 | F8 | IF-FW-ROAD-8J | Pendente / Realinhado por F8D1 | Coberto | Foundation avançada | RuntimeContentHandle + materialized instance + transition guard | Risco se release não for bottom-up | Release real vem depois do materializer local; Activity root deve liberar antes de Route root. |
...
| Runtime identity | Preservar typed; generalizar para todo spawned | F8 | F8 | IF-FW-ROAD-8B/F8C | F8C closed / PASS | Coberto | Foundation avançada | Typed identity policy | Baixo / controlado | RuntimeContentId, RuntimeContentOwner, RuntimeContentIdentity e RuntimeContentHandle existem como base passiva. |
...
| Destroy policy | Centralizar em runtime release execution | F8 | F8 | IF-FW-ROAD-8J | Pendente / Realinhado por F8D1 | Coberto | Foundation avançada | PrefabContentMaterializer + RuntimeContentHandle | Risco se Destroy ocorrer fora da release policy | Primeira policy pode destruir GameObject instanciado; pool return fica F11. |
