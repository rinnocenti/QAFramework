...
| Runtime release policy | Criar `RuntimeReleasePolicy` por escopo | F8 | F8 | IF-FW-ROAD-8J | Aplicado / Pendente compile-smoke | Coberto | Foundation avançada | Handle state + transition guard + release request/result | Risco de adapter físico capturar core depois | Release lógico por handle/escopo aplicado; Destroy, pool return, Addressables e scene unload ficam em adapters/trilhos específicos fora do core. |
