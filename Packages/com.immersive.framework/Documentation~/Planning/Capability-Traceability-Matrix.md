...
| Local content identity | Redesenhar como `LocalContentIdentity` | F5 | F5 | IF-FW-ROAD-5A, 5B | ADR aceito / tipo ausente | Coberto | Core | ActivityContentSet/RouteContentSet + typed identity | Risco de identity/discovery frágil | F5A aceitou identidade explícita: sem name/path/scene path/targetId universal como chave funcional. |
