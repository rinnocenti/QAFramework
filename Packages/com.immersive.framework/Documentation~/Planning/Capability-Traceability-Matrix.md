...
| Runtime Content Anchor binding | Criar `RuntimeContentAnchorBinding` | F9 | F9 | IF-FW-ROAD-9D/9E | Aplicado F9B | Coberto | Foundation avançada | Binding lógico resolve ContentAnchorSet + RuntimeContentHandle; placement físico ainda futuro | Risco controlado; sem lifecycle/placement físico ainda | Próximo: smoke/diagnóstico. |
...
| Runtime Content Anchor binding | Vincula logicamente RuntimeContentHandle a root/slot/point de Content Anchor; materialização física fica fora do core. | `RuntimeContentAnchorBinding` lógico | Aplicado F9B; placement físico futuro | F9 | IF-FW-ROAD-9D/9E |
...
| **F9** | Content Anchor binding request/result/content handle [F9A applied], RuntimeContentAnchorBinding lógico [F9B applied], spawn origin/slot e Content Anchor lifecycle policy ainda futuros |
