...
| Runtime Content Anchor binding | Criar `RuntimeContentAnchorBinding` e smoke dedicado | F9 | F9 | IF-FW-ROAD-9D/9E/9G | Aplicado F9C / smoke pending | Coberto | Foundation avançada | Binding lógico resolve ContentAnchorSet + RuntimeContentHandle; F9C valida bind idempotente, unbind e release lógico; placement físico ainda futuro | Risco controlado; sem lifecycle/placement físico ainda | Próximo: validar smoke e auditar próximo corte F9. |
...
| Runtime Content Anchor binding | Vincula logicamente RuntimeContentHandle a root/slot/point de Content Anchor; materialização física fica fora do core. | `RuntimeContentAnchorBinding` lógico + Content Anchor Binding Smoke | Aplicado F9C / smoke pending; placement físico futuro | F9 | IF-FW-ROAD-9D/9E/9G |
...
| **F9** | Content Anchor binding request/result/content handle [F9A applied], RuntimeContentAnchorBinding lógico [F9B applied], Content Anchor Binding Smoke [F9C applied / smoke pending], spawn origin/slot e Content Anchor lifecycle policy ainda futuros |
