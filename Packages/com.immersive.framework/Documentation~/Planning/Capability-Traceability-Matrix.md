...
| Runtime Content Anchor binding | Criar `RuntimeContentAnchorBinding` e smoke dedicado | F9 | F9 | IF-FW-ROAD-9D/9E/9G | F9E applied / compile pending | Coberto | Foundation avançada | Binding lógico resolve ContentAnchorSet + RuntimeContentHandle; F9C/F9D validam bind idempotente, unbind e release lógico; F9E torna o binding runtime owned pelo FrameworkRuntimeHost; placement físico ainda futuro | Risco controlado; sem cleanup automático/placement físico ainda | Próximo: validar compile e reexecutar Content Anchor Binding Smoke. |
...
| Runtime Content Anchor binding | Vincula logicamente RuntimeContentHandle a root/slot/point de Content Anchor; materialização física fica fora do core. | `RuntimeContentAnchorBinding` lógico + Content Anchor Binding Smoke | F9E applied / compile pending; placement físico futuro | F9 | IF-FW-ROAD-9D/9E/9G |
...
| Content Anchor lifecycle policy | Consumer/runtime libera binding antes de release lógico do runtime content. | `RuntimeContentAnchorBinding` cleanup/snapshots | F9E applied; cleanup automático futuro | F9 | IF-FW-ROAD-9H |
...
| **F9** | Content Anchor binding request/result/content handle [F9A applied], RuntimeContentAnchorBinding lógico [F9B applied], Content Anchor Binding Smoke [F9C PASS/F9D regression PASS], lifecycle cleanup/snapshots [F9D PASS], host-owned binding runtime [F9E applied], spawn origin/slot e placement adapters ainda futuros |
