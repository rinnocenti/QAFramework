...
| Readiness gate | Preservar como state/result | F4 | F4D | IF-FW-ROAD-4D | Presente mínimo / Pending smoke | Coberto | Core | Route handoff + ActivityContentSet | Baixo / controlado | F4D adiciona `ActivityReadinessState` mínimo: `Ready`, `None` e `NotReady`; sem gates visuais/actors/async. F4E reclassifica `ActivityContentBinding` como Activity Local Visibility Adapter para evitar leitura como materialização canônica. |
