...
| Lifecycle integration for runtime roots | Conectar criação explícita de roots/contextos ao lifecycle | F8 | F8 | IF-FW-ROAD-8F | Ausente / Próximo | Coberto / Next | Foundation avançada | RuntimeContentRuntime + RuntimeScopeContext | Médio / controlado | Route/Activity devem criar roots/contextos sem materializar prefab ainda. |
| Materialization request/result | Preservar como `RuntimeMaterializationRequest/Result` | F8 | F8 | IF-FW-ROAD-8G | Ausente / Planejado | Coberto | Foundation avançada | RuntimeContentRuntime + lifecycle context | Risco de API pública prematura | Reposicionado depois do owner/context para não nascer sem runtime owner. |
| Transition guard / scoped cancellation | Guardar transições e cancelamento por scope | F8 | F8 | IF-FW-ROAD-8H | Ausente / Planejado | Coberto | Foundation avançada | Materialization request/result | Médio / controlado | Evita materialização/release atravessando route/activity stale. |
| Prefab materializer | Preservar como `PrefabContentMaterializer` | F8 | F8 | IF-FW-ROAD-8I | Ausente / Planejado | Coberto | Foundation avançada | Request/result + transition guard | Risco de ownership/lifetime incompleto | Primeiro materializer concreto; ainda sem pool e sem Content Anchor. |
| Runtime release policy | Criar `RuntimeReleasePolicy` por escopo | F8 | F8 | IF-FW-ROAD-8J | Ausente / Planejado | Coberto | Foundation avançada | Prefab materializer + handle state | Risco de orphan se incompleto | Release físico centralizado por escopo; pool return continua fora de F8. |
...
| Destroy policy | Centralizar em `RuntimeReleasePolicy` | F8 | F8 | IF-FW-ROAD-8J | Ausente / Planejado | Coberto | Foundation avançada | Runtime release policy | Risco de orphan se incompleto | Destroy explícito só depois de request/result/materializer/release policy. |
...
| Lifecycle integration for runtime roots | Route/Activity criam roots/contextos explicitamente no lifecycle. | Teardown/setup espalhados | Integrar sem materializar prefab ainda | F8 | IF-FW-ROAD-8F |
...
| Transition guard / scoped cancellation | Impede operação stale/foreign durante troca de Route/Activity. | Guardas espalhadas na pipeline | Criar guarda antes do materializer concreto | F8 | IF-FW-ROAD-8H |
| Prefab materializer | Instancia prefab, registra handle, devolve result. | Adapters de actor/presentation | Preservar como `PrefabContentMaterializer` | F8 | IF-FW-ROAD-8I |
| Runtime release policy | Limpa instâncias ao sair do escopo (Activity exit, Route exit, Session shutdown). | Teardown stages espalhados | Criar `RuntimeReleasePolicy` por escopo | F8 | IF-FW-ROAD-8J |
...
| Destroy policy | Destroy explícito apenas quando pool return não se aplica. | `Destroy` espalhado (problema) | Centralizar em `RuntimeReleasePolicy` | F8 | IF-FW-ROAD-8J |
