...
| Materialization adapter boundary | Preservar como adapter físico fora do core | F8 | F8 | IF-FW-ROAD-8I | Ausente / Próximo | Coberto / Next | Foundation avançada | Request/result + transition guard | Risco de ownership/lifetime incompleto | Boundary explícita para adapters físicos externos ao core; prefab/cena/pool continuam fora do core. |
| Runtime release policy | Criar `RuntimeReleasePolicy` por escopo | F8 | F8 | IF-FW-ROAD-8J | Ausente / Planejado | Coberto | Foundation avançada | Handle state + transition guard | Risco de orphan lógico se incompleto | Release lógico por handle/escopo; Destroy, pool return, Addressables e scene unload ficam em adapters/trilhos específicos fora do core. |
