...
| F20 | `IN PROGRESS / F20C DIAGNOSTICS SMOKE APPLIED` | Pause State/Gate começou. F20B adiciona primitivas passivas de Pause; F20C adiciona smoke sintético de diagnostics. Ainda sem runtime request path, Gate real, overlay, input ou `Time.timeScale`. |
...
| F20 | Pause State and Pause Gate | Framework Core | `IN PROGRESS / F20C DIAGNOSTICS SMOKE APPLIED`: Pause como estado + Gate blocker. F20B adiciona request id, state, request/result, issue e snapshot passivos. F20C adiciona diagnostics smoke sintético. Não é Activity, menu, overlay, input system, `Time.timeScale` contract ou lifecycle de Route/Activity. |
...
F20B/F20C não autorizam Pause menu, overlay, input real, `Time.timeScale` adapter, loading screen, fade/curtain ownership, Pause como Activity ou lifecycle paralelo. F20C adiciona apenas smoke sintético. Setup visual/manual deve ficar para F21, exceto se um corte futuro declarar explicitamente o contrário e documentar os passos.
