...
| F20 | `IN PROGRESS / F20B PRIMITIVES APPLIED` | Pause State/Gate começou. F20B adiciona primitivas passivas de Pause; ainda sem runtime request path, Gate real, overlay, input ou `Time.timeScale`. |
...
| F20 | Pause State and Pause Gate | Framework Core | `IN PROGRESS / F20B PRIMITIVES APPLIED`: Pause como estado + Gate blocker. F20B adiciona request id, state, request/result, issue e snapshot passivos. Não é Activity, menu, overlay, input system, `Time.timeScale` contract ou lifecycle de Route/Activity. |
...
| F20C | `NEXT / PLANNED` | Smoke sintético de diagnostics de Pause: pause, resume, reject e idempotência. | Nenhum. |
...
F20B não autoriza Pause menu, overlay, input real, `Time.timeScale` adapter, loading screen, fade/curtain ownership, Pause como Activity ou lifecycle paralelo. Setup visual/manual deve ficar para F21, exceto se um corte futuro declarar explicitamente o contrário e documentar os passos.

### Próximo corte recomendado

```text
F20C - Pause Diagnostics Smoke
```
