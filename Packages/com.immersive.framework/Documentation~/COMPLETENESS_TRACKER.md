...
| F9 | `OPEN / F9F APPLIED / AUTOMATIC LOGICAL BINDING CLEANUP PENDING SMOKE` | F9E smoke passed; F9F cleans logical Content Anchor bindings automatically on Route/Activity owner exit before runtime root removal; compile/import and cleanup smoke pending | `Planning/F9Plus-Roadmap-Realignment.md`, `ContentAnchor/CONTENT_ANCHOR_BINDING_CONTRACTS.md`, `ContentAnchor/CONTENT_ANCHOR_BINDING_RUNTIME.md`, `ADRs/F9-content-anchor-binding-and-runtime-placement/` |
...
|---|---|
| Compile/import smoke | Confirms F9F lifecycle cleanup integration compiles in Unity. |
| Re-run: Content Anchor Binding Smoke | Regression check for host-owned logical anchor binding, idempotent rebind, unbind, logical release and unregister without physical placement. |
| Run: Content Anchor Binding Cleanup Smoke | Validates automatic logical binding cleanup during Route owner exit without physical placement or adapter execution. |
...
F8K is applied but F8 is not closed until validation passes. No further F8 feature work is authorized unless the F8K smoke exposes a defect.
