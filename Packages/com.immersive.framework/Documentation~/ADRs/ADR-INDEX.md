...
| F22 | [Loading Operation Progress Readiness Boundary](F22-ADR-LOADING-001-Loading-Operation-Progress-Readiness-Boundary.md) | Accepted / F22C Applied / F22D Next |
...
F22 Loading note: F22A accepts Loading as operation/progress/readiness reporting. It is not fade, curtain, loading screen prefab, TransitionEffect vocabulary or a SceneLifecycle replacement. F22A also reconciles existing loading-like concepts: SceneLifecycle still executes scene load/unload, Transition remains flow orchestration, and TransitionEffects `LoadingScreen` / `LoadingProgress` remain visual/effect-facing names rather than the canonical Loading model. F22B adds passive primitives. F22C adds pure weighted progress aggregation and `Run Loading Progress Aggregation Smoke`. Loading visual belongs to a later adapter boundary. Next cut: F22D - SceneLifecycle / Transition Loading Observation Adapter.
...
F22C applied note: F22C adds `LoadingProgressAggregationStatus`, `LoadingProgressAggregationResult`, `LoadingProgressAggregator` and `Run Loading Progress Aggregation Smoke` under QA Canvas `Show Loading diagnostics`. It does not add SceneLifecycle/Transition observation adapter, visual adapter, UI, fade, curtain, loading screen prefab or lifecycle execution. Next cut: F22D - SceneLifecycle / Transition Loading Observation Adapter.
