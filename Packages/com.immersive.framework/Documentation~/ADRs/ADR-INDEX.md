...
| F22 | [Loading Operation Progress Readiness Boundary](F22-ADR-LOADING-001-Loading-Operation-Progress-Readiness-Boundary.md) | Accepted / F22B Applied / F22C Next |
...
F22 Loading note: F22A accepts Loading as operation/progress/readiness reporting. It is not fade, curtain, loading screen prefab, TransitionEffect vocabulary or a SceneLifecycle replacement. F22A also reconciles existing loading-like concepts: SceneLifecycle still executes scene load/unload, Transition remains flow orchestration, and TransitionEffects `LoadingScreen` / `LoadingProgress` remain visual/effect-facing names rather than the canonical Loading model. Loading visual belongs to a later adapter boundary. Next cut: F22C - Loading Progress Aggregation Smoke.
...
F22A applied note: F22A is documentation-only. It accepts the Loading architecture plan, updates the roadmap/ADR index and records the audit that prevents loading ghosts/orphans across SceneLifecycle, Route Scene Composition, Transition and TransitionEffects. It adds no runtime code, asmdef changes, primitives, progress aggregator, smoke, fade, curtain, loading screen prefab, UI, scene object, prefab, ScriptableObject, SceneLifecycle replacement, Transition replacement, backend, PlayerPrefs or JSON. Next cut: F22C - Loading Progress Aggregation Smoke.


F22B applied note: F22B adds the canonical `Immersive.Framework.Loading` primitive namespace and `FrameworkIdentityDomain.Loading`. It creates operation id, step id, operation/step statuses, normalized progress, step weight, weighted progress, step and operation records. It does not add aggregation, smoke, SceneLifecycle/Transition observation adapter, visual adapter, UI, fade, curtain, loading screen prefab or lifecycle execution. Next cut: F22C - Loading Progress Aggregation Smoke.
