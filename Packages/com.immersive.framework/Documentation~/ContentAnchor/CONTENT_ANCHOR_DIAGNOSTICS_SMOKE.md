...
The Route smoke requests the configured Content Anchor Route and validates the `ContentAnchorDiscoveryResult` plus the route-local `ContentAnchorSet`. The Activity smoke requests the configured Route, lets the startup Activity run and validates `ActivityContentAnchorDiscoveryResult`.
