...
F20 - Pause State and Pause Gate / IN PROGRESS / F20A ADR PLAN ACCEPTED
...
```text
F20A is documentation/plan only. No scene object, Canvas, prefab or ScriptableObject is required.
F20 begins with the logical Pause core: state, request/result, snapshot/facts, policy and Gate blocker relationship.
Pause visual content, overlay and input binding are F21 boundaries.
F20B is the next cut: Pause State Primitives.
```
