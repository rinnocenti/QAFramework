# Audio QA Harness

Audio is one QA feature with one setup, Hub entry, primary scene and panel.

```text
1. Immersive Framework > QA > Setup > Audio > Configure Audio QA
2. Open Assets/ImmersiveFrameworkQA/Hub/Scenes/QA_Hub.unity
3. Enter Play Mode
4. Open Audio QA
5. Run All Audio QA
6. Inspect the consolidated PASS / FAIL output
```

The setup is idempotent and configures generated clips, Core Audio fixtures,
the primary `QA_Audio` scene, the internal Route B fixture and Hub entry. The
clip repair command remains maintenance-only.

## Internal groups

- Core Audio — `com.immersive.audio` provider proof.
- Framework BGM — optional bridge Route/Activity fixture.
- Framework BGM Regression — ADR-013A evidence; its deterministic controlled-provider
  fixture remains pending because the currently public bridge accepts only a concrete
  `AudioRuntimeHost` and exposes no injectable provider port.

The Framework BGM fixture uses official components only:

```text
FrameworkBgmDirector
FrameworkRouteBgmBinding
FrameworkActivityBgmBinding
FrameworkBgmActivityPolicy
RouteRequestTrigger
ActivityRequestTrigger
```

It does not use FIRSTGAME assets or consumer-specific BGM scripts.

## Generated Files

The builder creates or refreshes:

```text
Assets/ImmersiveFrameworkQA/Audio/Scenes/QA_Audio.unity
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_AudioDefaults.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_SfxCue.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_SfxCue_Pooled.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_SfxCue_MissingClip.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_AudioSfxPool.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_BgmCue.asset
Assets/ImmersiveFrameworkQA/Audio/AudioClips/QA_Sfx_Beep.wav
Assets/ImmersiveFrameworkQA/Audio/AudioClips/QA_Bgm_Tone.wav
Assets/ImmersiveFrameworkQA/Audio/Prefabs/QA_PooledAudioSource.prefab
```

The audio clips are synthetic sine tones generated by the scene builder. They are not final game assets.

The Framework BGM builder also creates or refreshes:

```text
Assets/ImmersiveFrameworkQA/Audio/Scenes/QA_AudioRouteB.unity
Assets/ImmersiveFrameworkQA/Audio/Routes/QA_FrameworkBgmRoute.asset
Assets/ImmersiveFrameworkQA/Audio/Routes/QA_FrameworkBgmRouteB.asset
Assets/ImmersiveFrameworkQA/Audio/Activities/QA_FrameworkBgmStartupActivity.asset
Assets/ImmersiveFrameworkQA/Audio/Activities/QA_FrameworkBgmOwnActivity.asset
Assets/ImmersiveFrameworkQA/Audio/Activities/QA_FrameworkBgmRetainPreviousActivity.asset
Assets/ImmersiveFrameworkQA/Audio/Activities/QA_FrameworkBgmRouteFallbackActivity.asset
Assets/ImmersiveFrameworkQA/Audio/Activities/QA_FrameworkBgmSilenceActivity.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_FrameworkBgm_RouteCue.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_FrameworkBgm_RouteBCue.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_FrameworkBgm_StartupActivityCue.asset
Assets/ImmersiveFrameworkQA/Audio/ScriptableObjects/QA_FrameworkBgm_ActivityCue.asset
```

## Scene Contents

`QA_Audio.unity` contains:

```text
Audio QA Root
  QA_AudioRuntimeHost
  QA_AudioRuntimeHost_MissingDefaults
  QA_AudioRuntimeHost_MissingPool
  QA_PoolRuntimeHost
  QA_AudioListenerRuntimeHost
  QA_AudioPanel
```

The panel uses IMGUI and does not require TextMeshPro or UGUI setup.

`QA_Audio.unity` contains Core Audio and the Framework BGM primary fixture.
`QA_AudioRouteB.unity` is internal Route B fixture infrastructure:

```text
QA_FrameworkBgmRoot_*
  QA_FrameworkBgm_AudioRuntimeHost
  FrameworkBgmDirector
  FrameworkRouteBgmBinding
QA_FrameworkBgm_Activity_*
  ActivityLocalVisibilityAdapter
  FrameworkActivityBgmBinding
QA_FrameworkBgmPanel_*
  FrameworkBgmQaPanel
  RouteRequestTrigger / ActivityRequestTrigger children
```

## Panel Buttons

- `Compose Runtime Host` - validates settings resolution from `AudioDefaultsAsset`.
- `Play SFX` - plays the direct SFX cue.
- `Play BGM` - plays the BGM cue.
- `Stop BGM` - stops the active BGM.
- `Run Direct SFX Smoke` - validates SFX playback result and handle creation.
- `Run Missing Clip Smoke` - validates explicit `FailedMissingClip`.
- `Run Missing Defaults Smoke` - validates explicit `FailedMissingDefaults` when the host has no defaults.
- `Run Pooled SFX Smoke` - validates explicit pooled SFX with `PoolRuntimeHost`.
- `Run Missing Pool Smoke` - validates explicit `FailedMissingPoolService`.
- `Run Listener Smoke` - validates `AudioListenerRuntimeHost` report-only duplicate detection.
- `Run BGM Smoke` - validates BGM play and stop.
- `Run All Audio QA` - runs the Core Audio suite; Framework BGM controls remain in the same panel.
- `Reset QA Counters` - clears panel counters.

The Framework BGM group validates this manual sequence:

```text
1. Startup Activity BGM
2. Clear Activity: Route BGM
3. Activity Own BGM
4. Activity Without BGM: Retain Previous
5. Activity UseRoute: Route Fallback
6. Activity Silence
7. Clear Activity After Silence
8. Request Own Activity BGM Again
9. Route Switch Clears Retained Activity BGM
```

For each mutation, inspect `Last Operation`, `Previous Confirmed BGM`, `Requested BGM`,
`Resulting Confirmed BGM` and `Reason`. `Applied` and `Released` are valid only after the
audio provider confirms the operation; `NoChange` must not add a provider mutation.

To certify the negative execution matrix, use an explicitly invalid/missing configured
audio authority or cue in an isolated QA copy and verify that `Rejected` or
`OptionalAuthorityUnavailable` preserves the previous confirmed cue. Repeating the same
request must produce a new provider attempt. Do not treat an authored request as confirmed
state after a rejection.

## Expected PASS Criteria

With `Run All Audio QA`:

```text
[AUDIO_QA] PASS. Direct SFX smoke ...
[AUDIO_QA] PASS. Missing clip smoke ...
[AUDIO_QA] PASS. Missing defaults smoke ...
[AUDIO_QA] PASS. Pooled SFX smoke ...
[AUDIO_QA] PASS. Missing pool smoke ...
[AUDIO_QA] PASS. Listener smoke ...
[AUDIO_QA] PASS. BGM smoke ...
[AUDIO_QA] PASS. All Audio QA smokes completed. passed='7' failed='0'.
```

The exact order may include intermediate PASS lines from individual button operations.

With the Framework BGM fixture, expected logs include:

```text
[FRAMEWORK_BGM] Route BGM set
[FRAMEWORK_BGM] Startup Activity BGM intent dispatched
[FRAMEWORK_BGM] Activity BGM set
[FRAMEWORK_BGM] BGM operation completed
[FRAMEWORK_BGM] Activity BGM policy Silence applied
```

## What This QA Proves

- `AudioDefaultsAsset` serializes and is consumed by `AudioRuntimeHost`.
- SFX direct playback returns `Succeeded` with a valid handle.
- SFX pooled playback returns `Succeeded` with a valid handle and returns rented sources to the pool.
- BGM playback and stop return framework-owned typed execution evidence.
- Cue missing clip returns `FailedMissingClip`.
- Missing defaults returns `FailedMissingDefaults`; no silent fallback is used.
- Missing pool returns `FailedMissingPoolService`; no direct fallback is used.
- `AudioListenerRuntimeHost` reports duplicates and does not destroy or disable them when policy is `ReportOnly`.
- `com.immersive.audio` can be consumed without `com.immersive.framework`.

The Framework BGM fixture proves:

- Route BGM is applied through `FrameworkRouteBgmBinding`.
- Startup Activity BGM is pre-applied through an explicit `FrameworkActivityBgmBinding`.
- Activity-owned BGM wins over Route BGM.
- Activity without BGM can retain the previous Activity BGM until Route exit.
- `UseRoute` falls back to Route BGM.
- `Silence` calls `StopBgm`.
- Clear Activity returns to the expected Route or retained policy result.
- Route switch clears retained Activity BGM.
- Desired BGM and provider-confirmed BGM are shown separately; rejected intent is not
  retained as a restoration target.

## What This QA Does Not Test

- AudioMixer routing. Current routing is metadata resolution only.
- Final game UI or FIRSTGAME usage.
