...
{
    public enum QaLoadingPresentationEvidenceKind
    {
        Unknown = 0,
        RequestReceived = 1,
        VisibleApplied = 2,
    ...
    public readonly struct QaLoadingPresentationEvidenceEntry
    {
        public QaLoadingPresentationEvidenceEntry(
            int sequence,
            int frame,
            ...
            int progressPercent)
        {
            ...
            ProgressValue01 = progressSupported
                ? Mathf.Clamp01(progressValue01)
                : 0f;
            ProgressPercent = progressSupported
                ? Mathf.Clamp(progressPercent, 0, 100)
                : 0;
        }
        ...
        public QaLoadingPresentationEvidenceKind Kind { get; }
        ...
        public float ProgressValue01 { get; }
    ...
    {
        private const int MaximumPresentationEvidenceEntries = 256;
        ...
        private readonly List<QaLoadingPresentationEvidenceEntry> presentationEvidence =
        ...
        public int ShowRequestCount { get; private set; }
        public int UpdateRequestCount { get; private set; }
        public int HideRequestCount { get; private set; }
        public int VisibleApplyCount { get; private set; }
        public int HiddenApplyCount { get; private set; }
        public int ResultEvidenceCount { get; private set; }
        public IReadOnlyList<QaLoadingPresentationEvidenceEntry> PresentationEvidence =>
            presentationEvidence;

        public event Action<QaLoadingPresentationEvidenceEntry>
            PresentationEvidenceRecorded;
        ...
        public void ResetPresentationEvidence()
        {
            ...
            bool recordResult = true)
        {
            ...
            {
                return recordResult ? Record(failureResult) : Record(failureResult, false);
            }

            if (visibleState)
            {
                CancelPendingHide();
                ApplyVisibleState(resolvedCanvasGroup, request, recordEvidence: true);
                LoadingSurfaceResult result = LoadingSurfaceResult.SucceededResult(
                    request,
                    AdapterName,
                    $"Adapter '{AdapterName}' applied visible loading state.");
                return recordResult ? Record(result) : Record(result, false);
            }

            CancelPendingHide();
            ApplyHiddenState(resolvedCanvasGroup, request, recordEvidence: true);
            LoadingSurfaceResult hiddenResult = LoadingSurfaceResult.SucceededResult(
                request,
                AdapterName,
                $"Adapter '{AdapterName}' applied hidden loading state.");
            return recordResult ? Record(hiddenResult) : Record(hiddenResult, false);
        }
            ...
            LoadingSurfaceAction expectedAction)
        {
            ...
            var result = Apply(request, expectedAction, true, recordResult: false);
        ...
        {
            ApplyVisibleState(resolvedCanvasGroup, null, recordEvidence: false);
        }
            ...
            bool recordEvidence)
        {
            ...
            if (recordEvidence && request.HasValue)
            {
                VisibleApplyCount++;
                RecordEvidence(
        ...
        private void ApplyHiddenState(
            CanvasGroup resolvedCanvasGroup,
            LoadingSurfaceRequest? request = null,
            bool recordEvidence = false)
        {
        ...
        private void RecordRequestReceived(LoadingSurfaceRequest request)
        {
            ...
            RecordEvidence(
                ...
                request,
                LoadingSurfaceResultStatus.Unknown,
                "Loading surface request received.");
        }
            ...
            string message)
        {
            ...
            var entry = new QaLoadingPresentationEvidenceEntry(
                ...
                request.ProgressSupported,
                request.ProgressSupported
                    ? request.Progress.NormalizedValue
                    : 0f,
                request.ProgressSupported
                    ? request.Progress.PercentRounded
                    : 0);
            presentationEvidence.Add(entry);
            PresentationEvidenceRecorded?.Invoke(entry);
        }

        private LoadingSurfaceResult Record(
            LoadingSurfaceResult result,
            bool recordPresentationEvidence = true)
        {
            lastStatus = result.Status;
            lastMessage = result.Message;
            lastVisibleState = HasCanvasGroup && CurrentAlpha > 0.001f;
            if (!recordPresentationEvidence)
            {
                return result;
            }
