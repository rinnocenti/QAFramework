...
{
    /// <summary>
    /// Edit Mode regression for Pause/Input/Gate authoring composition.
    /// UnityPlayerInputGateAdapter owns PlayerInput and the Gameplay map;
    /// PlayerPauseInput authors only the Pause action.
    /// </summary>
    public static class PlayerQaPauseCompositionRegression
    {
        private const string Prefix = "[P0_PAUSE_INPUT_GATE_COMPOSITION]";
        private const string MenuPath =
            "Immersive Framework/QA/Player/Pause/Run Pause Input Gate Composition";
        ...
        private static void RunPauseAuthoringSurfaceCase(ICollection<UnityEngine.Object> created)
        {
            ...
            Require(serialized.FindProperty("pauseAction") != null,
                "PlayerPauseInput must author Pause Action.");
            Require(serialized.FindProperty("playerInput") == null,
                "PlayerPauseInput must not serialize a duplicate PlayerInput authority.");
            Require(serialized.FindProperty("gameplayActionMap") == null,
                "PlayerPauseInput must not serialize a duplicate Gameplay Action Map authority.");
            Require(serialized.FindProperty("gameplayActionMapName") == null,
                "PlayerPauseInput must not retain the legacy Gameplay Action Map name authority.");
        }

        private static void RunGateAuthoritySurfaceCase(ICollection<UnityEngine.Object> created)
        {
            ...
            Require(serialized.FindProperty("restorePreviousState") != null,
                "UnityPlayerInputGateAdapter must retain explicit restore-state authoring.");
        }

        private static void RunGateWithoutPauseCase(ICollection<UnityEngine.Object> created)
        {
            ...
            Require(host.GetComponent<PlayerPauseInput>() == null,
                "Gate-only proof accidentally materialized PlayerPauseInput.");
            Require(gate.TryValidateAuthoring(out string diagnostic),
                "UnityPlayerInputGateAdapter must be valid without PlayerPauseInput. " + diagnostic);
            Require(ReferenceEquals(gate.PlayerInput, playerInput),
                "Gate Adapter did not retain its explicit PlayerInput authority.");
            Require(gate.GameplayActionMapReference.HasSameIdentity(fixture.GameplayMap),
                "Gate Adapter did not retain the authored Gameplay Action Map identity.");
        }

        private static void RunPauseDerivesGateAuthorityCase(ICollection<UnityEngine.Object> created)
        {
            ...
            Require(gate.TryValidateAuthoring(out string gateDiagnostic),
                "Gate Adapter authoring must be valid before Pause validation. " + gateDiagnostic);
            Require(pauseInput.TryValidateAuthoring(out string pauseDiagnostic),
                "PlayerPauseInput must validate from the co-located Gate authority. " + pauseDiagnostic);
            Require(ReferenceEquals(pauseInput.PlayerInput, playerInput),
                "PlayerPauseInput did not derive PlayerInput from the Gate Adapter.");
            Require(pauseInput.GameplayActionMapReference.HasSameIdentity(fixture.GameplayMap),
                "PlayerPauseInput did not derive Gameplay Action Map identity from the Gate Adapter.");
        }

        private static void RunMissingGateCase(ICollection<UnityEngine.Object> created)
        {
            ...
            PlayerPauseInput pauseInput = ConfigurePause(host, fixture.PauseReference);
            Require(!pauseInput.TryValidateAuthoring(out string diagnostic),
            ...
            Require(pauseInput.PlayerInput == null,
        ...
        {
            Require(
                Attribute.IsDefined(typeof(UnityPlayerInputGateAdapter), typeof(DisallowMultipleComponent), true),
                "UnityPlayerInputGateAdapter must prevent duplicate authored adapters on one GameObject.");
        }

        private static void RunGuidIdentityCase(ICollection<UnityEngine.Object> created)
        {
            ...
            UnityPlayerInputGateAdapter gate = ConfigureGate(host, playerInput, authored.GameplayMap);
            Require(string.Equals(authored.GameplayMap.name, target.GameplayMap.name, StringComparison.Ordinal),
                "GUID identity proof requires equal map names.");
            Require(authored.GameplayMap.id != target.GameplayMap.id,
            ...
            Require(diagnostic.IndexOf("Name fallback is not used", StringComparison.Ordinal) >= 0,
                "Gameplay map rejection did not prove GUID-only resolution. " + diagnostic);
        }

        private static void RunRestoreIdempotenceCase(ICollection<UnityEngine.Object> created)
        {
            ...
            gate.Restore();
            gate.Restore();
            Require(!gate.IsBlockedByAdapter,
                "Repeated Restore on an unblocked Gate Adapter must remain idempotently unblocked.");
        }

        private static ActionFixture CreateActionFixture(ICollection<UnityEngine.Object> created, string name)
        {
            ...
            return new ActionFixture(asset, globalMap, gameplayMap, pauseReference);
        }

        private static GameObject CreateInactiveHost(ICollection<UnityEngine.Object> created, string name)
        {
        ...
        private static PlayerInput ConfigurePlayerInput(GameObject host, InputActionAsset actions)
        {
            ...
            InputActionMap gameplayMap)
        {
            ...
            mapReference.FindPropertyRelative("actionAsset").objectReferenceValue = gameplayMap.asset;
            mapReference.FindPropertyRelative("actionMapId").stringValue = gameplayMap.id.ToString("D");
            mapReference.FindPropertyRelative("cachedActionMapName").stringValue = gameplayMap.name;
            serialized.ApplyModifiedPropertiesWithoutUndo();
            return gate;
        }

        private static PlayerPauseInput ConfigurePause(GameObject host, InputActionReference pauseReference)
        {
            PlayerPauseInput pauseInput = host.AddComponent<PlayerPauseInput>();
            var serialized = new SerializedObject(pauseInput);
        ...
        private static void DestroyCreated(IReadOnlyList<UnityEngine.Object> created)
        {
