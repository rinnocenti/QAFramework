    ...
    public static class PlayerQaSceneBuilder
    {
        ...
        internal static InputActionAsset EnsureInputActions()
        {
            InputActionAsset existing =
                AssetDatabase.LoadAssetAtPath<InputActionAsset>(PlayerQaPaths.InputActionsPath);
            if (HasRequiredInputActions(existing))
            {
                return existing;
            }
        ...
        private static bool HasRequiredInputActions(InputActionAsset asset)
        {
            if (asset == null)
            {
                return false;
            ...
            InputActionMap global = asset.FindActionMap("Global", false);
            InputAction activate = gameplay?.FindAction("Activate", false);
            InputAction pause = global?.FindAction("Pause", false);
            return activate != null &&
                activate.type == InputActionType.Button &&
                HasBinding(activate, "<Keyboard>/space") &&
                pause != null &&
                pause.type == InputActionType.Button &&
                HasBinding(pause, "<Keyboard>/escape");
        }
