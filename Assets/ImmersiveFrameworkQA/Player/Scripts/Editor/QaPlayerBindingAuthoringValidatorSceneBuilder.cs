...
{
    public static class QaPlayerBindingAuthoringValidatorSceneBuilder
    {
        private const string Root = "Assets/ImmersiveFrameworkQA/Player";
        private const string ScenePath = Root + "/Scenes/QA_PlayerBindingAuthoringValidator.unity";
        private const string RoutePath = Root + "/Routes/QA_PlayerBindingAuthoringValidatorRoute.asset";
        private const string ActivityPath = Root + "/Activities/QA_PlayerBindingAuthoringValidatorActivity.asset";

        [MenuItem("Immersive Framework QA/Player/Create or Refresh F50A Player Binding Authoring Validator QA Scene")]
        public static void CreateOrRefreshPlayerBindingAuthoringValidatorScene()
        {
            EnsureFolders();

            ActivityAsset activity = CreateActivityAsset(
                ActivityPath,
                "QA Player Binding Authoring Validator Activity",
                "F50A Player binding authoring validator QA activity.");

            CreateRouteAsset(
                RoutePath,
                "QA Player Binding Authoring Validator Route",
                ScenePath,
                "F50A Player binding authoring validator QA route.",
                activity);

            Scene scene = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
            scene.name = "QA_PlayerBindingAuthoringValidator";

            UnityEngine.Camera mainCamera = CreateCamera("QA_PlayerBindingAuthoringValidatorCamera", new Color(0.028f, 0.036f, 0.052f, 1f));

            GameObject root = new GameObject("QA_PlayerBindingAuthoringValidator_Root");
            ...
            GameObject viewTarget = new GameObject("QA_PlayerBindingAuthoringValidator_ViewTarget");
            viewTarget.transform.SetParent(root.transform, false);
            viewTarget.transform.localPosition = new Vector3(0f, 1.5f, 0f);

            GameObject controlTarget = new GameObject("QA_PlayerBindingAuthoringValidator_ControlTarget");
            ...
            GameObject fixtureObject = new GameObject("QA_F50A_PlayerBindingAuthoringValidator");
            QaPlayerBindingAuthoringValidatorFixture fixture = fixtureObject.AddComponent<QaPlayerBindingAuthoringValidatorFixture>();
            ...
            Debug.Log("[F50A_PLAYER_BINDING_AUTHORING_VALIDATOR_QA] Fixture scene ready: " + ScenePath);
        }

        [MenuItem("Immersive Framework QA/Player/Open F50A Player Binding Authoring Validator QA Scene")]
        public static void OpenPlayerBindingAuthoringValidatorScene()
        {
            if (!File.Exists(ToFullPath(ScenePath)))
            {
                CreateOrRefreshPlayerBindingAuthoringValidatorScene();
                return;
            }
        ...
        private static void ConfigurePlayerSlotDeclaration(PlayerSlotDeclaration declaration)
        {
            ...
            SetString(serialized, "displayName", "QA Player Binding Authoring Slot");
            SetString(serialized, "reason", "qa.player-binding-authoring-validator.slot-declaration");
        ...
        {
            SerializedObject serialized = new SerializedObject(declaration);
            SetString(serialized, "actorId", "qa.player-binding-authoring-validator.actor");
            SetString(serialized, "displayName", "QA Player Binding Authoring Actor");
            SetString(serialized, "reason", "qa.player-binding-authoring-validator.actor-declaration");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(declaration);
        }
            ...
            ActorDeclaration actorDeclaration)
        {
            ...
            SetString(serialized, "occupiedActorId", "qa.player-binding-authoring-validator.actor");
            SetString(serialized, "displayName", "QA Player Binding Authoring Occupancy");
            SetString(serialized, "reason", "qa.player-binding-authoring-validator.occupancy");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            ...
            ActorReadinessBehaviour readinessBehaviour)
        {
            ...
            SetString(serialized, "initialReason", "qa.player-binding-authoring-validator.entry.initial");
            ...
            Transform viewTarget)
        {
            ...
            SetString(serialized, "initialReason", "qa.player-binding-authoring-validator.view.initial");
            SetBool(serialized, "rebuildOnAwake", true);
            serialized.ApplyModifiedPropertiesWithoutUndo();
            ...
            Transform controlTarget)
        {
            ...
            SetString(serialized, "inputSourceId", "qa.input.binding-authoring-validator.intent");
            SetEnum(serialized, "initialState", PlayerControlState.Declared);
            SetString(serialized, "initialSuspensionReason", string.Empty);
            SetString(serialized, "initialReason", "qa.player-binding-authoring-validator.control.initial");
            SetBool(serialized, "rebuildOnAwake", true);
            serialized.ApplyModifiedPropertiesWithoutUndo();
        ...
        private static void ConfigureFixture(
            QaPlayerBindingAuthoringValidatorFixture fixture,
