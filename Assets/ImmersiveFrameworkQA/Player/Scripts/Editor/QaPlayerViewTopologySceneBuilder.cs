...
{
    public static class QaPlayerViewTopologySceneBuilder
    {
        private const string Root = "Assets/ImmersiveFrameworkQA/Player";
        private const string ScenePath = Root + "/Scenes/QA_PlayerViewTopology.unity";
        private const string RoutePath = Root + "/Routes/QA_PlayerViewTopologyRoute.asset";
        private const string ActivityPath = Root + "/Activities/QA_PlayerViewTopologyActivity.asset";

        [MenuItem("Immersive Framework QA/Player/Create or Refresh F49H PlayerView Topology QA Scene")]
        public static void CreateOrRefreshPlayerViewTopologyScene()
        {
            EnsureFolders();

            ActivityAsset activity = CreateActivityAsset(
                ActivityPath,
                "QA PlayerView Topology Activity",
                "F49H PlayerView topology passive validation QA activity.");

            CreateRouteAsset(
                RoutePath,
                "QA PlayerView Topology Route",
                ScenePath,
                "F49H PlayerView topology passive validation QA route.",
                activity);

            Scene scene = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
            scene.name = "QA_PlayerViewTopology";

            UnityEngine.Camera qaCamera = CreateCamera("QA_PlayerViewTopology_Camera", new Color(0.035f, 0.04f, 0.07f, 1f));

            GameObject target = new GameObject("QA_PlayerViewTopology_Target");
            ...
            GameObject targetAnchor = new GameObject("QA_PlayerViewTopology_TargetAnchor");
            ...
            GameObject fixtureObject = new GameObject("QA_F49H_PlayerViewTopology");
            QaPlayerViewTopologyFixture fixture = fixtureObject.AddComponent<QaPlayerViewTopologyFixture>();
            ConfigureFixture(fixture, slotDeclaration, occupancy, readinessBehaviour, entryBehaviour, viewBehaviour);

            EditorSceneManager.SaveScene(scene, ScenePath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log("[F49H_PLAYER_VIEW_TOPOLOGY_QA] Fixture scene ready: " + ScenePath);
        }

        [MenuItem("Immersive Framework QA/Player/Open F49H PlayerView Topology QA Scene")]
        public static void OpenPlayerViewTopologyScene()
        {
            if (!File.Exists(ToFullPath(ScenePath)))
            {
                CreateOrRefreshPlayerViewTopologyScene();
                return;
            }
        ...
        private static void ConfigurePlayerSlotDeclaration(PlayerSlotDeclaration declaration)
        {
            ...
            SetString(serialized, "displayName", "QA PlayerView Topology Slot");
            SetString(serialized, "reason", "qa.player-view-topology.slot-declaration");
        ...
        {
            SerializedObject serialized = new SerializedObject(declaration);
            SetString(serialized, "actorId", "qa.player-view-topology.actor");
            SetString(serialized, "displayName", "QA PlayerView Topology Actor");
            SetString(serialized, "reason", "qa.player-view-topology.actor-declaration");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(declaration);
        }
            ...
            ActorDeclaration actorDeclaration)
        {
            ...
            SetString(serialized, "occupiedActorId", "qa.player-view-topology.actor");
            SetString(serialized, "displayName", "QA PlayerView Topology Occupancy");
            SetString(serialized, "reason", "qa.player-view-topology.occupancy");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            ...
            ActorReadinessBehaviour readinessBehaviour)
        {
            ...
            SetString(serialized, "initialReason", "qa.player-view-topology.entry.initial");
            ...
            PlayerEntryBehaviour entryBehaviour)
        {
            ...
            SetString(serialized, "initialReason", "qa.player-view-topology.view.initial");
            SetBool(serialized, "rebuildOnAwake", true);
            serialized.ApplyModifiedPropertiesWithoutUndo();
        ...
        private static void ConfigureFixture(
            QaPlayerViewTopologyFixture fixture,
            PlayerSlotDeclaration slotDeclaration,
            PlayerSlotOccupancy occupancy,
            ActorReadinessBehaviour readinessBehaviour,
            PlayerEntryBehaviour entryBehaviour,
