...
{
    public static class QaPlayerViewBindingAdapterSceneBuilder
    {
        private const string Root = "Assets/ImmersiveFrameworkQA/Player";
        private const string ScenePath = Root + "/Scenes/QA_PlayerViewBindingAdapter.unity";
        private const string RoutePath = Root + "/Routes/QA_PlayerViewBindingAdapterRoute.asset";
        private const string ActivityPath = Root + "/Activities/QA_PlayerViewBindingAdapterActivity.asset";

        [MenuItem("Immersive Framework QA/Player/Create or Refresh F51A PlayerView Binding Adapter QA Scene")]
        public static void CreateOrRefreshPlayerViewBindingAdapterScene()
        {
            EnsureFolders();

            ActivityAsset activity = CreateActivityAsset(
                ActivityPath,
                "QA PlayerView Binding Adapter Activity",
                "F51A PlayerView binding adapter QA activity.");

            CreateRouteAsset(
                RoutePath,
                "QA PlayerView Binding Adapter Route",
                ScenePath,
                "F51A PlayerView binding adapter QA route.",
                activity);

            Scene scene = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
            scene.name = "QA_PlayerViewBindingAdapter";

            UnityEngine.Camera mainCamera = CreateCamera("QA_PlayerViewBindingAdapterCamera", new Color(0.024f, 0.04f, 0.052f, 1f));

            GameObject root = new GameObject("QA_PlayerViewBindingAdapter_Root");
            ...
            PlayerViewBindingTargetBehaviour bindingTarget = root.AddComponent<PlayerViewBindingTargetBehaviour>();

            GameObject viewTarget = new GameObject("QA_PlayerViewBindingAdapter_ViewTarget");
            viewTarget.transform.SetParent(root.transform, false);
            viewTarget.transform.localPosition = new Vector3(0f, 1.5f, 0f);

            GameObject controlTarget = new GameObject("QA_PlayerViewBindingAdapter_ControlTarget");
            ...
            GameObject fixtureObject = new GameObject("QA_F51A_PlayerViewBindingAdapter");
            QaPlayerViewBindingAdapterFixture fixture = fixtureObject.AddComponent<QaPlayerViewBindingAdapterFixture>();
            ...
            Debug.Log("[F51A_PLAYERVIEW_BINDING_ADAPTER_QA] Fixture scene ready: " + ScenePath);
        }

        [MenuItem("Immersive Framework QA/Player/Open F51A PlayerView Binding Adapter QA Scene")]
        public static void OpenPlayerViewBindingAdapterScene()
        {
            if (!File.Exists(ToFullPath(ScenePath)))
            {
                CreateOrRefreshPlayerViewBindingAdapterScene();
                return;
            }
        ...
        private static void ConfigurePlayerSlotDeclaration(PlayerSlotDeclaration declaration)
        {
            ...
            SetString(serialized, "displayName", "QA PlayerView Binding Adapter Slot");
            SetString(serialized, "reason", "qa.playerview-binding-adapter.slot-declaration");
        ...
        {
            SerializedObject serialized = new SerializedObject(declaration);
            SetString(serialized, "actorId", "qa.playerview-binding-adapter.actor");
            SetString(serialized, "displayName", "QA PlayerView Binding Adapter Actor");
            SetString(serialized, "reason", "qa.playerview-binding-adapter.actor-declaration");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(declaration);
        }
            ...
            ActorDeclaration actorDeclaration)
        {
            ...
            SetString(serialized, "occupiedActorId", "qa.playerview-binding-adapter.actor");
            SetString(serialized, "displayName", "QA PlayerView Binding Adapter Occupancy");
            SetString(serialized, "reason", "qa.playerview-binding-adapter.occupancy");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            ...
            ActorReadinessBehaviour readinessBehaviour)
        {
            ...
            SetString(serialized, "initialReason", "qa.playerview-binding-adapter.entry.initial");
            ...
            Transform viewTarget)
        {
            ...
            SetString(serialized, "initialReason", "qa.playerview-binding-adapter.view.initial");
            SetBool(serialized, "rebuildOnAwake", true);
            serialized.ApplyModifiedPropertiesWithoutUndo();
            ...
            Transform controlTarget)
        {
            ...
            SetString(serialized, "inputSourceId", "qa.input.playerview-binding-adapter.intent");
            SetEnum(serialized, "initialState", PlayerControlState.Declared);
            SetString(serialized, "initialSuspensionReason", string.Empty);
            SetString(serialized, "initialReason", "qa.playerview-binding-adapter.control.initial");
            SetBool(serialized, "rebuildOnAwake", true);
            serialized.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(control);
        }

        private static void ConfigureBindingTarget(PlayerViewBindingTargetBehaviour target)
        {
