...
{
    public static class QaPlayerControlBindingAdapterSceneBuilder
    {
        private const string Root = "Assets/ImmersiveFrameworkQA/Player";
        private const string ScenePath = Root + "/Scenes/QA_PlayerControlBindingAdapter.unity";
        private const string RoutePath = Root + "/Routes/QA_PlayerControlBindingAdapterRoute.asset";
        private const string ActivityPath = Root + "/Activities/QA_PlayerControlBindingAdapterActivity.asset";

        [MenuItem("Immersive Framework QA/Player/Create or Refresh F52A PlayerControl Binding Adapter QA Scene")]
        public static void CreateOrRefreshPlayerControlBindingAdapterScene()
        {
            EnsureFolders();

            ActivityAsset activity = CreateActivityAsset(
                ActivityPath,
                "QA PlayerControl Binding Adapter Activity",
                "F52A PlayerControl binding adapter QA activity.");

            CreateRouteAsset(
                RoutePath,
                "QA PlayerControl Binding Adapter Route",
                ScenePath,
                "F52A PlayerControl binding adapter QA route.",
                activity);

            Scene scene = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
            scene.name = "QA_PlayerControlBindingAdapter";

            UnityEngine.Camera mainCamera = CreateCamera("QA_PlayerControlBindingAdapterCamera", new Color(0.024f, 0.04f, 0.052f, 1f));

            GameObject root = new GameObject("QA_PlayerControlBindingAdapter_Root");
            ...
            PlayerControlBindingTargetBehaviour bindingTarget = root.AddComponent<PlayerControlBindingTargetBehaviour>();

            GameObject viewTarget = new GameObject("QA_PlayerControlBindingAdapter_ViewTarget");
            viewTarget.transform.SetParent(root.transform, false);
            viewTarget.transform.localPosition = new Vector3(0f, 1.5f, 0f);

            GameObject controlTarget = new GameObject("QA_PlayerControlBindingAdapter_ControlTarget");
            ...
            ConfigureBindingTarget(bindingTarget);

            GameObject fixtureObject = new GameObject("QA_F52A_PlayerControlBindingAdapter");
            QaPlayerControlBindingAdapterFixture fixture = fixtureObject.AddComponent<QaPlayerControlBindingAdapterFixture>();
            ConfigureFixture(fixture, root, readinessBehaviour, entryBehaviour, viewBehaviour, controlBehaviour, bindingTarget);

            EditorSceneManager.SaveScene(scene, ScenePath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log("[F52A_PLAYERCONTROL_BINDING_ADAPTER_QA] Fixture scene ready: " + ScenePath);
        }

        [MenuItem("Immersive Framework QA/Player/Open F52A PlayerControl Binding Adapter QA Scene")]
        public static void OpenPlayerControlBindingAdapterScene()
        {
            if (!File.Exists(ToFullPath(ScenePath)))
            {
                CreateOrRefreshPlayerControlBindingAdapterScene();
                return;
            }
        ...
        private static void ConfigurePlayerSlotDeclaration(PlayerSlotDeclaration declaration)
        {
            ...
            SetString(serialized, "displayName", "QA PlayerControl Binding Adapter Slot");
            SetString(serialized, "reason", "qa.playercontrol-binding-adapter.slot-declaration");
        ...
        {
            SerializedObject serialized = new SerializedObject(declaration);
            SetString(serialized, "actorId", "qa.playercontrol-binding-adapter.actor");
            SetString(serialized, "displayName", "QA PlayerControl Binding Adapter Actor");
            SetString(serialized, "reason", "qa.playercontrol-binding-adapter.actor-declaration");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(declaration);
        }
            ...
            ActorDeclaration actorDeclaration)
        {
            ...
            SetString(serialized, "occupiedActorId", "qa.playercontrol-binding-adapter.actor");
            SetString(serialized, "displayName", "QA PlayerControl Binding Adapter Occupancy");
            SetString(serialized, "reason", "qa.playercontrol-binding-adapter.occupancy");
            serialized.ApplyModifiedPropertiesWithoutUndo();
            ...
            ActorReadinessBehaviour readinessBehaviour)
        {
            ...
            SetString(serialized, "initialReason", "qa.playercontrol-binding-adapter.entry.initial");
            ...
            Transform viewTarget)
        {
            ...
            SetString(serialized, "initialReason", "qa.playercontrol-binding-adapter.view.initial");
            SetBool(serialized, "rebuildOnAwake", true);
            serialized.ApplyModifiedPropertiesWithoutUndo();
            ...
            Transform controlTarget)
        {
            ...
            SetString(serialized, "inputSourceId", "qa.input.playercontrol-binding-adapter.intent");
            SetEnum(serialized, "initialState", PlayerControlState.Declared);
            SetString(serialized, "initialSuspensionReason", string.Empty);
            SetString(serialized, "initialReason", "qa.playercontrol-binding-adapter.control.initial");
            SetBool(serialized, "rebuildOnAwake", true);
            serialized.ApplyModifiedPropertiesWithoutUndo();
            EditorUtility.SetDirty(control);
        }

        private static void ConfigureBindingTarget(PlayerControlBindingTargetBehaviour target)
        {
            ...
            PlayerControlBindingTargetBehaviour bindingTarget)
        {
            ...
            SetObject(serialized, "bindingTarget", bindingTarget);
