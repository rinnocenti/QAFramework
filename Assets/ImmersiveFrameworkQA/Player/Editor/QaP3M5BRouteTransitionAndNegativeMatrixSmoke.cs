...
namespace ImmersiveFrameworkQA.Player.Editor
{
    ...
    public static class QaP3M5BRouteTransitionAndNegativeMatrixSmoke
    {
        ...
        {
            (LocalPlayerHostAuthoring PhysicalHost, Transform PhysicalActor) retainedPhysical =
                await RunAsync(releaseScenePlayer: false);
        ...
        /// <summary>
        /// Causal first-acquisition rollback proof. The invalid Activity enters through the
        /// same automatic SceneProvided lifecycle as a normal Route, but its authored Actor
        /// evidence intentionally disagrees with its selected profile before any Session
        /// physical commit can occur.
        /// </summary>
        internal static async Task RunFailedFirstSceneAdoptionAsync()
        {
            Component runtimeHost = await AwaitUniqueReadyQaRuntimeHostAsync();
            ...
            PlayerSlotId slotId = ResolveFirstConfiguredSlot().PlayerSlotId;
            RouteAsset hubRoute = LoadAsset<RouteAsset>(HubRoutePath);
            RouteAsset failedRoute = LoadAsset<RouteAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.FailedFirstAdoptionRoutePath);
            RouteAsset validRoute = LoadAsset<RouteAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteAPath);
            ActivityAsset validActivity = LoadAsset<ActivityAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteAActivityPath);
            RuntimeContentOwner physicalOwner = CreateSessionPhysicalOwner();

            try
            {
                await NormalizeToHubAsync(runtimeHost, hubRoute, "failed-first-adoption-normalize");
                AssertInitialHubState(runtimeHost, preparationModule, sceneAdmissionModule, slotId);

                object failedRequest = await RequestRouteAsync(
                    runtimeHost,
                    failedRoute,
                    "failed-first-scene-adoption");
                AssertRouteAdmissionFailed(
                    failedRequest,
                    "Failed first SceneProvided adoption unexpectedly reached Activity readiness.");

                SceneLocalPlayerAdmissionAuthoring candidate =
                    await AwaitRejectedSurfaceAsync(
                        QaP3M5BRouteTransitionAndNegativeMatrixSetup
                ...
                AssertNotNull(originalActor,
                    "Failed first SceneProvided adoption lost its original Actor reference.");
                AssertEqual(1, ResolveSurfaces(
                        QaP3M5BRouteTransitionAndNegativeMatrixSetup
                            .FailedFirstAdoptionActivityScenePath).Length,
                    "Failed first SceneProvided adoption cloned the candidate admission surface.");
                AssertFalse(candidate.HasActiveAdmission,
                    "Failed first SceneProvided adoption retained a stale contextual admission.");
                AssertFalse(originalHost.IsJoined,
                    "Failed first SceneProvided adoption transferred the original Host to Session membership.");
                AssertEqual(1, originalHost.ActorMount.childCount,
                    "Failed first SceneProvided adoption replaced or destroyed the original Actor mount content.");
                AssertSame(originalActor.transform, originalHost.ActorMount.GetChild(0),
                    "Failed first SceneProvided adoption replaced the original Actor object.");
                AssertInitialHubState(runtimeHost, preparationModule, sceneAdmissionModule, slotId);

                object validRequest = await RequestRouteAsync(
                    runtimeHost,
                    validRoute,
                    "failed-first-scene-adoption-recover-valid");
                AssertRequestSucceeded(validRequest,
                    "A valid first SceneProvided adoption did not recover after the rejected candidate.");
                AssertRouteActivityReady(validRequest,
                    "A valid first SceneProvided adoption did not become ready after rejected candidate rollback.");
                LoadedPlayerFixture recovered = await AwaitActiveFixtureAsync(
                    QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteAActivityScenePath,
                    runtimeHost,
                    validActivity,
                    preparationModule,
            ...
            {
                await NormalizeToHubAsync(runtimeHost, hubRoute, "failed-first-adoption-cleanup");
            }
        }

        /// <summary>
        /// Causal post-commit rollback proof. A valid Route A commits the physical Player,
        /// then an invalid Route B fails during contextual SceneProvided admission. The
        /// failure must retire B-only state without releasing the already committed Session
        /// physical occurrence.
        /// </summary>
        internal static async Task RunFailedContextualReprojectionAsync()
        {
            Component runtimeHost = await AwaitUniqueReadyQaRuntimeHostAsync();
            object preparationModule = ResolveHostComponent(
                runtimeHost,
            ...
            PlayerSlotId slotId = ResolveFirstConfiguredSlot().PlayerSlotId;
            RouteAsset hubRoute = LoadAsset<RouteAsset>(HubRoutePath);
            RouteAsset routeA = LoadAsset<RouteAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteAPath);
            RouteAsset failedRoute = LoadAsset<RouteAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.FailedReprojectionRoutePath);
            RouteAsset recoveryRoute = LoadAsset<RouteAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteBPath);
            ActivityAsset routeAActivity = LoadAsset<ActivityAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteAActivityPath);
            ActivityAsset failedActivity = LoadAsset<ActivityAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.FailedReprojectionActivityPath);
            ActivityAsset recoveryActivity = LoadAsset<ActivityAsset>(
                QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteBActivityPath);
            RuntimeContentOwner physicalOwner = CreateSessionPhysicalOwner();

            try
            {
                await NormalizeToHubAsync(runtimeHost, hubRoute, "failed-reprojection-normalize");
                object routeARequest = await RequestRouteAsync(
                    runtimeHost,
                    routeA,
                    "failed-reprojection-enter-a");
                AssertRequestSucceeded(routeARequest,
                    "Failed contextual reprojection proof could not establish valid Route A.");
                AssertRouteActivityReady(routeARequest,
                    "Failed contextual reprojection proof Route A is not ready.");
                LoadedPlayerFixture committed = await AwaitActiveFixtureAsync(
                    QaP3M5BRouteTransitionAndNegativeMatrixSetup.RouteAActivityScenePath,
                    runtimeHost,
                    routeAActivity,
                    preparationModule,
                ...
                Quaternion actorRotation = physicalActor.rotation;
                PlayerParticipationSnapshot committedSnapshot =
                    CreateParticipationSnapshot(runtimeHost);
                PlayerSlotRuntimeSnapshot committedSlot = FindSlot(committedSnapshot, slotId);
                ...
                AssertRouteAdmissionFailed(
                    failedRequest,
                    "Invalid Route B contextual reprojection unexpectedly reached Activity readiness.");
                SceneLocalPlayerAdmissionAuthoring failedSurface =
                    await AwaitRejectedSurfaceAsync(
                        QaP3M5BRouteTransitionAndNegativeMatrixSetup
                ...
                {
                    AssertRetiredContextualAdmission(committed, "Route A before failed B");
                }
                AssertCommittedPhysicalStatePreserved(
                    committed,
                    preparationModule,
                    slotId,
                    physicalOwner,
                ...
                object recoveryRequest = await RequestRouteAsync(
                    runtimeHost,
                    recoveryRoute,
                    "failed-contextual-reprojection-recover-valid");
                AssertRequestSucceeded(recoveryRequest,
                    "Valid recovery after failed contextual reprojection did not succeed.");
                AssertRouteActivityReady(recoveryRequest,
                    "Valid recovery after failed contextual reprojection did not become ready.");
                LoadedPlayerFixture recovered = await AwaitActiveFixtureAsync(
                ...
                AssertFreshContextualAdmission(
                    committed,
                    recovered,
                    "Failed B rollback -> valid B recovery");
            ...
            {
                await NormalizeToHubAsync(runtimeHost, hubRoute, "failed-reprojection-cleanup");
            }
        }

        /// <summary>
        /// The canonical normal A->B->A proof observes the exact physical object,
        /// preparation token, adoption revision and RuntimeContent identity. It therefore
        /// proves no candidate, handoff or replacement was required without depending on a
        /// diagnostic shortcut such as NotRequired.
        /// </summary>
        internal static Task RunNoPhysicalHandoffOnActivityTransitionAsync() =>
            RunAsync(releaseScenePlayer: false);

        private static async Task<(LocalPlayerHostAuthoring PhysicalHost, Transform PhysicalActor)>
            RunAsync(bool releaseScenePlayer)
        {
            ...
            return (admittedPhysicalHost, admittedPhysicalActor);
        }
        ...
        private static async Task NormalizeToHubAsync(
            Component runtimeHost,
            RouteAsset hubRoute,
            string reason)
        {
            AssertNotNull(runtimeHost,
                "P3M5B route normalization requires a Framework runtime host.");
            AssertNotNull(hubRoute,
                "P3M5B route normalization requires the QA Hub Route.");
            if (ReferenceEquals(ResolveCurrentRoute(runtimeHost), hubRoute))
            {
                return;
            }

            object request = await RequestRouteAsync(runtimeHost, hubRoute, reason);
            AssertRequestSucceeded(request,
                "P3M5B could not normalize to the QA Hub Route.");
            AssertRouteActivityReady(request,
                "P3M5B QA Hub Route did not become ready during normalization.");
        }

        private static void AssertRouteAdmissionFailed(
            object routeRequest,
            string message)
        {
        ...
        private static async Task<SceneLocalPlayerAdmissionAuthoring>
            AwaitRejectedSurfaceAsync(string scenePath, string caseName)
        {
            ...
            string transition)
        {
            AssertTrue(TryGetAdoptionToken(
                ...
                transition + " released the committed SceneProvided adoption.");
            AssertTrue(TryGetPreparationSummary(
                    preparationModule,
                    playerSlotId,
                    out PlayerActorPreparationSummary preparation) &&
                preparation.IsPrepared,
                transition + " released the committed physical Player preparation.");
            AssertEqual(expected.Adoption, adoption,
                transition + " created a second physical adoption.");
            AssertEqual(expected.Adoption.AdoptionRevision, adoption.AdoptionRevision,
                transition + " changed the committed physical adoption revision.");
            AssertEqual(expected.Preparation.Token, preparation.Token,
                transition + " changed the committed physical preparation token.");
            AssertEqual(expected.Preparation.Materialization.RuntimeContentIdentity,
                preparation.Materialization.RuntimeContentIdentity,
                transition + " changed committed physical RuntimeContent.");
            AssertEqual(expectedPhysicalOwner, preparation.Materialization.Owner,
                transition + " moved committed physical ownership out of Session.");
            AssertEqual(expected.Adoption.ActorId, preparation.Materialization.ActorId,
                transition + " changed the committed physical ActorId.");
            AssertPhysicalRepresentationPreserved(
                expectedHost,
                expectedActor,
            ...
            PlayerSlotRuntimeSnapshot currentSlot = FindSlot(
                CreateParticipationSnapshot(participationContext),
                playerSlotId);
            AssertTrue(currentSlot.IsJoined,
                transition + " implicitly left the committed Session Slot.");
            AssertEqual(expectedSlot.Revision, currentSlot.Revision,
                transition + " changed the committed Session Slot occurrence revision.");
            AssertEqual(expectedSlot.SelectionRevision, currentSlot.SelectionRevision,
                transition + " reselected the committed physical Actor.");
            AssertEqual(expectedSlot.SelectedActorProfileId, currentSlot.SelectedActorProfileId,
                transition + " changed the persistent selected Actor identity.");
        }
