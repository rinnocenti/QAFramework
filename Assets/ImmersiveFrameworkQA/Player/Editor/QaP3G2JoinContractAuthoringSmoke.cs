...
namespace ImmersiveFrameworkQA.Player.Editor
{
    ...
    internal static class QaP3G2JoinContractAuthoringSmoke
    {
        ...
        internal static void Run()
        {
            ...
            try
            {
                ...
                GameObject validPrefab = CreateTechnicalHost(
                    ...
                    includeMount: true);
                AssignHostPrefab(authoring, manager, validPrefab);
                ...
                manager.notificationBehavior = PlayerNotifications.InvokeUnityEvents;
                object invalidNotificationReport = Validate(authoring, application);
                AssertHasErrors(invalidNotificationReport, "Non-C# join notifications were accepted.");
                AssertReportContains(invalidNotificationReport, "C# event");
                completed.Add("invalid-notification-behavior-rejected");

                manager.notificationBehavior = PlayerNotifications.InvokeCSharpEvents;
                AssignHostPrefab(authoring, manager, null);
                ...
                AssignHostPrefab(authoring, manager, missingInputPrefab);
                ...
                AssignHostPrefab(authoring, manager, missingHostPrefab);
                ...
                GameObject invalidMountPrefab = CreateTechnicalHost(
                    ...
                    includeMount: false);
                AssignHostPrefab(authoring, manager, invalidMountPrefab);
                ...
                AssignAuthoredHostPrefab(authoring, validPrefab);
                manager.playerPrefab = null;
                string emptyManagerBefore = EditorJsonUtility.ToJson(manager);
                object emptyManagerReport = Validate(authoring, application);
                AssertNoErrors(emptyManagerReport,
                    "Valid authoring with an empty technical manager was rejected.");
                AssertEqual(emptyManagerBefore, EditorJsonUtility.ToJson(manager),
                    "Editor validation materialized or otherwise mutated the empty manager.");
                completed.Add("empty-manager-valid-and-non-mutating");
                ...
                SetTechnicalCapacity(manager, 1);
                object incompatibleCapacityReport = Validate(authoring, application);
                AssertHasErrors(incompatibleCapacityReport,
                    "Manager capacity below configured Slot count was accepted.");
                AssertReportContains(incompatibleCapacityReport, "maxPlayerCount");
                completed.Add("incompatible-capacity-rejected");
                SetTechnicalCapacity(manager, 2);

                GameObject divergentPrefab = CreateTechnicalHost(
                    created,
                    "QA P3G2 Divergent Local Player Host",
                    includeMount: true);
                manager.playerPrefab = divergentPrefab;
                object divergentPrefabReport = Validate(authoring, application);
                AssertHasErrors(divergentPrefabReport,
                    "Divergent manager Local Player Host Prefab was accepted.");
                AssertReportContains(divergentPrefabReport, "diverges");
                AssertSame(divergentPrefab, manager.playerPrefab,
                    "Validation silently overwrote the divergent manager prefab.");
                completed.Add("divergent-manager-prefab-rejected-without-mutation");
        ...
        private static void AssignHostPrefab(
            LocalPlayerProvisioningAuthoring authoring,
            PlayerInputManager manager,
            GameObject prefab)
        {
        ...
        private static void AssignAuthoredHostPrefab(
            LocalPlayerProvisioningAuthoring authoring,
            GameObject prefab)
        {
            var serialized = new SerializedObject(authoring);
