...
namespace ImmersiveFrameworkQA.Lifecycle.Editor
{
    public static class QaLifecycleSceneBuilder
    {
        ...
        private static bool CreateRouteScene(string scenePath, string label, string routePath, Color backgroundColor, Vector3 contentOffset)
        {
            ...
            EnsureComponent<QaRouteContentLifecycleProbe>(routeRoot);
        ...
        private static bool CreateAdditionalScene()
        {
            ...
            ActivityAsset activityA = LoadAsset<ActivityAsset>(ActivityAPath);
            ActivityAsset activityB = LoadAsset<ActivityAsset>(ActivityBPath);
            if (routeA == null || activityA == null || activityB == null)
            {
                Debug.LogError("[QA_LIFECYCLE_SETUP] Additional Route scene setup aborted because Route A or Activity A/B is missing.");
                return false;
            }
            ...
            SetSerialized(routeBinding, "requiredness", 10);
            EnsureComponent<QaRouteContentLifecycleProbe>(root);
            CreateActivityContent(scene, "QA_Lifecycle_Additional_ActivityA", activityA, new Vector3(-0.8f, 1.2f, 0f), Color.green, PrimitiveType.Sphere);
            CreateActivityContent(scene, "QA_Lifecycle_Additional_ActivityB_InactiveChild", activityB, new Vector3(0.8f, 1.2f, 0f), Color.yellow, PrimitiveType.Capsule, initiallyActive: false);
        ...
        private static void SetSerialized(Object target, string propertyName, Object value)
        {
            ...
            if (property == null)
            {
                throw new System.InvalidOperationException($"[QA_LIFECYCLE_SETUP] Serialized property not found. target='{target.GetType().Name}' property='{propertyName}'.");
            }
        ...
        private static void SetSerializedActivities(ActivityVisibilityRule target, ActivityAsset activity)
        {
            if (activity == null)
            {
                throw new System.InvalidOperationException("Activity content requires a valid Activity asset.");
            }
