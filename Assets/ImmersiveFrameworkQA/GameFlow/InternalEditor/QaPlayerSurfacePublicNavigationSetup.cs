    ...
    internal static class QaPlayerSurfacePublicNavigationSetup
    {
        ...
        internal static void PrepareForCertification()
        {
            ...
            try
            {
                ...
                ConfigureActivityContentFixture(slot);
                Require(
        ...
        {
            Require(
                slot != null && slot.PlayerSlotId.IsValid,
                "Player Surface Activity content requires the explicit primary Player Slot.");
            ...
            foreach (GameObject candidate in content.GetRootGameObjects())
            {
                ...
                if (candidate != null &&
                    string.Equals(candidate.name, "ActivityPlayerInitialPlacement", StringComparison.Ordinal))
                {
