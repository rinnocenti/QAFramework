    ...
    public static class QaBootGameFlowBaselineRegression
    {
        ...
        [MenuItem(MenuPath)] private static async void Run()
        {
            ...
            try
            {
                ...
                ActivityLocalVisibilityAdapter adapterA = ResolveAdapter(AdditionalScene, activityA); ActivityLocalVisibilityAdapter adapterB = ResolveAdapter(AdditionalScene, activityB); Require(!adapterB.gameObject.activeSelf, "Activity B Additional adapter must start inactive."); Require(host.State.CurrentActivity != null && host.State.CurrentActivity.HasSameIdentity(activityA), "Route A startup Activity is not A."); Require(adapterA.gameObject.activeSelf && !adapterB.gameObject.activeSelf, "Startup Activity visibility diverged."); done.Add("startup-activity-a-and-inactive-b");
        ...
        private static Evidence ResolveBinding(string path, RouteAsset route) { Scene scene = Scene(path); var matches = new List<RouteContentBinding>(); foreach (GameObject root in scene.GetRootGameObjects()) foreach (RouteContentBinding binding in root.GetComponentsInChildren<RouteContentBinding>(true)) if (binding != null && binding.Route != null && binding.Route.HasSameIdentity(route) && binding.MatchesRoute(route)) matches.Add(binding); Require(matches.Count == 1, $"Expected one Route binding in '{path}'."); QaRouteContentLifecycleProbe[] probes = matches[0].GetComponentsInChildren<QaRouteContentLifecycleProbe>(true); Require(probes.Length == 1, $"Expected one probe in '{path}'."); return new Evidence(matches[0], probes[0]); }
        private static ActivityLocalVisibilityAdapter ResolveAdapter(string path, ActivityAsset activity) { Scene scene = Scene(path); var matches = new List<ActivityLocalVisibilityAdapter>(); foreach (GameObject root in scene.GetRootGameObjects()) foreach (ActivityLocalVisibilityAdapter adapter in root.GetComponentsInChildren<ActivityLocalVisibilityAdapter>(true)) if (adapter != null && adapter.TryGetSingleActivityOwner(out ActivityAsset owner) && owner.HasSameIdentity(activity) && adapter.HasExplicitLocalContentId) matches.Add(adapter); Require(matches.Count == 1, $"Expected one Activity adapter in '{path}'."); return matches[0]; }
        ...
        private readonly struct Evidence { internal Evidence(RouteContentBinding binding, QaRouteContentLifecycleProbe probe) { Binding = binding; Probe = probe; } internal RouteContentBinding Binding { get; } internal QaRouteContentLifecycleProbe Probe { get; } }
    }
}
