...
namespace ImmersiveFrameworkQA.ActivityFlow.Editor
{
    public static class QaActivityLocalVisibilityRuleRegression
    {
        ...
        private static readonly FieldInfo ActivitiesField = typeof(ActivityVisibilityRule)
        ...
        public static void Run()
        {
            ...
            try
            {
                var adapter = root.AddComponent<ActivityVisibilityRule>();
                int cases = 0;
                ...
                Debug.Log($"[QA_ACTIVITY_LOCAL_VISIBILITY_RULE] status='Passed' cases='{cases}' completed='positive,negative,no-active,invalid,idempotent'.");
            }
        ...
        private static void Configure(ActivityVisibilityRule adapter, ActivityAsset[] activities, ActivityVisibilityMatchMode mode, ActivityVisibilityNoActivePolicy policy)
        {
        ...
        private static void Check(ActivityVisibilityRule adapter, ActivityAsset activity, bool valid, bool visible, string name, ref int cases) { var result=adapter.EvaluateVisibility(activity); Assert(result.IsValid==valid && result.DesiredVisibility==visible,name); cases++; }
        private static void CheckInvalid(ActivityVisibilityRule adapter, ActivityAsset activity, string name, ref int cases, string expectedReason = null) { bool state=adapter.gameObject.activeSelf; string before=EditorJsonUtility.ToJson(adapter); var result=adapter.EvaluateVisibility(activity); Assert(!result.IsValid && !string.IsNullOrWhiteSpace(result.DiagnosticReason) && (expectedReason == null || result.DiagnosticReason == expectedReason) && state==adapter.gameObject.activeSelf && before==EditorJsonUtility.ToJson(adapter),name); cases++; }
    }
}
