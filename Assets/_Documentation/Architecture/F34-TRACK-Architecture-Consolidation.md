...
| FLOWTRIGGER - Request/Trigger/State | Implemented locally / pending Unity validation | FLOWTRIGGER-1 | `F42-ADR-FlowTrigger-Request-State-Helper.md` accepts a neutral non-MonoBehaviour helper under `Runtime/Common/FlowTriggers`. `PauseRequestTrigger` and `PauseInputActionRuntimeBridgeTrigger` use it for local state/diagnostics. Route/Activity triggers were inspected but left untouched because they live under `Runtime/GameFlow`, which F42 did not allow changing. | `GAMEFLOW` and `LIFECYCLE-KERNEL-REMAINING` remain pending. Future GameFlow-scoped trigger migration may migrate Route/Activity request triggers. | `FLOWTRIGGER-VALIDATION` | Pending Unity compile/import, Standard Smoke, Pause InputAction Runtime Bridge Trigger Smoke and Pause Runtime PlayerInput Bridge Smoke. |
...
| Flow triggers | Implemented locally / pending Unity validation | Bridge, Consumer, Validator/Evidence |
...
1. `FLOWTRIGGER-VALIDATION`
2. `LIFECYCLE-KERNEL-REMAINING`
3. `GAMEFLOW`
...
- Owner Unity validation for `FLOWTRIGGER-1`.
