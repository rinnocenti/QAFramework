...
| LIFECYCLE - Route/Activity Operation Kernel | Partial / evidence kernel implemented locally | LIFECYCLE-C, C1, D, E, F, LIFECYCLE-KERNEL-1 | Scope-tail release/cleanup mechanics closed. `F44-ADR-Lifecycle-Operation-Evidence-Kernel.md` adds lifecycle-local operation evidence projection for Route/Activity request logs. | Full lifecycle orchestration kernel, content dispatch kernel, readiness kernel and GameFlow envelope remain pending. | `LIFECYCLE-KERNEL-2` after F44 validation | F44 pending Unity import/compile and real Route/Activity log validation. |
...
| Lifecycle operation evidence kernel | Implemented locally / pending Unity validation | `F44-ADR-Lifecycle-Operation-Evidence-Kernel.md`, `Runtime/Common/LifecycleOperations/*`, `FrameworkRuntimeHost` Route/Activity log projection. | Lifecycle-local operation evidence kernel created. Route and Activity request diagnostics project `lifecycleOperation*` stage names, statuses, issue counts, blocking counts, side-effect count, failed/skipped stage counts and diagnostic text. | Full lifecycle orchestration kernel, content dispatch kernel, readiness kernel and GameFlow envelope remain pending. | `LIFECYCLE-KERNEL-2` after validation | Pending Unity import/compile, Standard Smoke, Activity Baseline Smoke, Route Scene Composition Smoke, Route Release Smoke, Composite Lifecycle Release Smoke and real Route/Activity log inspection. |
...
| RouteLifecycle, ActivityFlow, SceneLifecycle | Partial / operation evidence kernel implemented locally | Operation Service, Consumer, Validator/Evidence |
...
- F44 owner Unity validation and whether `LIFECYCLE-KERNEL-2` should target orchestration, content dispatch or readiness evidence next.
